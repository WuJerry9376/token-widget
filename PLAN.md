...
- 遗留小项：加油包"到期日"数据源未透出（sources 无字段）；console 变体不挂版本资源且不经 spec（调试专用）；auth.json 明文 OAuth（项目根+dist 副本）转 DPAPI 一键与 refresh 自动续期为二期候选；dev 机两凭据待续（用户明示不管）；**门面素材待刷新**（efb58b8 版含「窗口」旧文案+v1.8.3 foot，v1.9.0 后统一重拍入库）

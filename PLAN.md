...
- **GitHub 发布（进行中）**：仓库内容级密钥扫描 CLEAN（仅测试桩 fake cookie 命中样式，无真实凭据）；gh CLI 2.101 已装、**待用户 `gh auth login`**（设备码流程）后：建仓（拟 public，匿名拉 release 前提）→ push main+tags → slug 入 config DEFAULTS → **v1.7.0 重建** → `gh release create v1.7.0 dist\TokenWidget.exe` → 发布流程写入 build/README

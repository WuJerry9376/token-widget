...
- **M21 官方图标素材轮 + v1.8.2 门面波次收口（2026-09-19，fixer→打包线）**：M21（素材轮，无 git 无 build）——foot 自绘剪影整删禁回流、改官方 Invertocat（AGENTS.md 共享资产 Black 版只读源，tools/make_gh_assets.py 幂等烘焙 FAINT/SOFT×四档 8 PNG，PAPER 底不透明避 Tk alpha/缩放两坑；(master,档) memo 双态预载；素材缺失=图标隐藏面板不崩；spec datas 打入、gh_asset_dir frozen=_MEIPASS/assets）；u26 二次改造+capture_m18 更名产物 m21_ghicon.png，**169 保持**。收口 v1.8.2=M21 commit `68323ea` + README 门面三处（Disclaimer & known limitations / 杀软厂商注记 EN+中 / A6→v1.8.2+Releases 外链）+ 双源升版 + **frozen 资产实证新判据**（windowed --open-settings + EnumWindows 裁 foot：FAINT 墨 242px 图标实渲 PASS，`local\m21_frozen_foot.png`；⚠️ 顺产发现：console 变体裸命令不经 spec=不含 assets，需 `--add-data` 才带素材——已记 build/README，交付无碍暂不改）；门禁 169、常驻双进程、注册表 IDENTICAL、MpCmdRun 0 检出、守卫四件一致 → tag v1.8.2+push+gh release。
- **M23+M24+M24C 已发布（v1.8.3，2026-09-20）**：M23 百炼套餐到期提示（`Usage.plan_end` 通用字段仅百炼填，C 行尾「· 套餐 10-11 到期」≤7d 橙/≤3d 红/跨年全日期/stale 随灰/tooltip 全式，Go/Codex 恒 None 零占位）；M24 四修（↻ 层序统一 `_rot_raise` 收口、Go 日窗映射+三主条 5h/日/周 固定序、高光改绝对阈 P(24)、全画面去"窗"字）；M24C Go `~5h`→`5h`；基线 172→177；v1.8.3 commit `5847531`+release 2124893、tag+push+gh release 上架、frozen windowed 冒烟 PASS（数据态渲染判据=同代码路径 capture_m24 实拍+门禁矩阵双层覆盖，边界已注）。门面模拟图用户已验收（m24 版）
- **M25+M26 已发布（v1.9.0，2026-09-21）**：M25 全画面用户可见「窗口」字样清零（含百炼行/tooltip/面板/验证消息，渲染串级联断言，docstring 技术语境保留）；M26 **DPI Per-Monitor v2** 重构——PMv2 感知+250ms `GetDpiForWindow` 轮询（子类化崩溃面规避，选型理由在 ui 头）、负像素字号统一 helper、clamp/reclamp、foot 图标四档重选、真机双 96 屏跨屏实渲+量化剖面（梯度能量+42.6%/中间调拖影−27%）、frozen 交付形态 procValue=2 实证；fix-8 state 事故恢复落位重启核验 PASS；新基线 **186/186**（+test_m26_dpi 9）、Release v1.9.0 上架、commit b372c94+188261b。**遗留**：真 150% 档跨屏实渲待混缩硬件（脚本 tools/diag/diag_m26_xscreen 已入库随时补跑）；虚拟屏未动（缩放改动需管理员，本机会话非提权；禁用同理——用户可在设备管理器一键禁用或提权会话我来）
- **M28 已发布（v1.9.1 收口，2026-09-18）**：更新链去 cmd 化重启——旧 `restart_update.cmd` 分离态父链触发「Security validation failure」弹窗且掐死 start（替换其实成功、体感像失败），整体退役；新链=写 `pending_swap.json` marker → DETACHED Popen staged 新 exe 本体（`--post-update-swap --start-marker`，零 shell 零可见窗口）→ 新实例启动早期改名序列（old→.old 占用重试→**改名自身 .new→正名**→删 marker）；失败落 FAILED.txt 走既有橙字/提权链、staged 位置照常运行不 brick；双向兼容（无 marker 零副作用）+ marker TTL 自清（本进程即 marker.new 例外执行）+ .old 残留自修复（`_image_holder_alive` 映像占用判定，禁误搬回）。基线 **187/187**（M15 套 29→34）；commit `13aa2ae`+`ef96b47`，双源升版看门 PASS；**1.9.1 正式体 E2E 复跑六判据全 PASS**（零 cmd/零 WER/marker 消费/正名替换/改名后旧进程存活/自修复不触发，SWAPPED@+1.0s）；frozen 冒烟+守卫四件+注册表 IDENTICAL+MpCmdRun 0 检出；tag v1.9.1 + Release 上架（asset digest==dist 全等）。过渡指引已落 README A6 双处与 release notes：**v1.9.0→v1.9.1 本次升级仍由旧脚本执行，或再见一次弹窗/未自动重启=替换已成功双击即新版；自 1.9.1 起全程干净**
- dev 机两凭据待续（Codex CLI 重登/重贴 token；百炼 Cookie 重贴）——用户明示不管；**M27 门面素材已刷新至 v1.9.0 真态**（40eccf3 入库，cv2 模板匹配 20 件零「窗口」命中；capture_m22 断言同步 93ae084，186/186）

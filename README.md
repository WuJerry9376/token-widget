...
- **v1.8.0（当前发布版）**：设置页 foot 行版本签名左侧新增 **GitHub 剪影图标**（一键在浏览器打开仓库页；零 API/零凭据，未配置仓库时隐藏；拉起浏览器失败橙字指路）；更新链加入镜像备用源 + SHA-256 完整性校验（信任锚=官方 API）。上代 v1.7.2 的「☆ 加星」按钮按用户反馈改向并入此图标；频控命中状态行保持「上次检查：HH:MM」精简档。

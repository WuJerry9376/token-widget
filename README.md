...
- 设置页「更新」分组：☑自动更新（默认开；**检查触发点=程序启动后 ~5s、打开设置页时、每日 5 点后首帧**，统一 6 小时频控合并）、当前版本、「检查更新」手动按钮（无视频控）。
...
- **v1.7.2（当前发布版）**：更新分组新增「☆ 给本项目加星」一键在浏览器打开 GitHub 项目页（非 star API，本程序零网络请求零凭据接触；未配置仓库时按钮禁用）；频控命中的状态行精简为「上次检查：HH:MM」。
